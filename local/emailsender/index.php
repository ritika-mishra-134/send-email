...
if (has_capability('local/emailsender:managefiles', context_system::instance())) {
    ...
    $returnurl = new moodle_url('/local/emailsender/index.php');
    ...
    if (empty($iid)) {
        if ($mform->is_cancelled()) {
            redirect($CFG->wwwroot . '/local/emailsender/index.php', 'Form has been cancelled');
    ...
    if (!in_array('email', $filecolumns)) {
        throw new \moodle_exception('fieldrequired', '', $returnurl, get_string('email', 'local_emailsender'));
    }
    if (!in_array('firstname', $filecolumns)) {
        throw new \moodle_exception('fieldrequired', '', $returnurl, get_string('firstname', 'local_emailsender'));
    }
    if (!in_array('lastname', $filecolumns)) {
        throw new \moodle_exception('fieldrequired', '', $returnurl, get_string('lastname', 'local_emailsender'));
    }
    // Second Form.
    ...
    if ($formdata = $mform2->is_cancelled()) {
        ...
        while ($linenum <= $previewrows && $fields = $cir->next()) {
            ...
            if ($csvfields['firstname'] && $csvfields['lastname'] && $csvfields['email']) {
                ...
                $emailsendertask->set_custom_data([
                    'firstname' => $firstname,
                    'lastname' => $lastname,
                    'email' => $email,
                ]);
