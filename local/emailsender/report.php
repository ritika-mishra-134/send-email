...
if (has_capability('local/emailsender:managefiles', context_system::instance())) {
    ...
    $userecords = $DB->get_records('emaillogs');
    echo $OUTPUT->header();
    $table = new html_table();
    $table->head = [
        get_string('sno', 'local_emailsender'),
        get_string('fullname', 'local_emailsender'),
        get_string('email', 'local_emailsender'),
        get_string('status', 'local_emailsender'),
        get_string('time', 'local_emailsender'),
    ];
    $i = 0;
    if ($userecords) {
        foreach ($userecords as $record) {
            $i++;
            $table->data[] = [
                'Sno.' => $i,
